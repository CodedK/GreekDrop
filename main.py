...
def transcribe_file():
    ...
    try:
        ...
        if selected_format == "txt":
            out_path = base_path + "_transcript.txt"
            with open(out_path, "w", encoding="utf-8") as f:
                f.write(result["text"])
        else:
            out_path = base_path + f"_subtitles.{selected_format}"
...
btn = tk.Button(frame, text="📂 Επέλεξε αρχείο ήχου", command=transcribe_file, font=("Arial", 14))
btn.grid(row=0, column=0, padx=10)

format_var = tk.StringVar(value="txt")
format_menu = ttk.Combobox(frame, textvariable=format_var, values=["txt", "srt", "vtt"], width=10, font=("Arial", 12))
